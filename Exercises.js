...
let listNumOfDaysInYear = new Map();
listNumOfDaysInYear.set("Earth", 365);
listNumOfDaysInYear.set("Mars", 288);
let GetAgeInYears = (_ageInSeconds, _planet) => {
...
let CountWordOccurence = (_phrase) => {
    let _word_array = _phrase.split(' ');
    let i = 0;
    let _word_occurrence_map = new Map();
...
let ReverseString = (_name) => {
    console.log(_name.split('').reverse().join(''));
};
ReverseString("Firoz");
console.log("----------------------Exercise 6-------------------------");
class Triangle {
    constructor(_side1, _side2, _side3) {
        this.side_1 = _side1;
        this.side_2 = _side2;
        this.side_3 = _side3;
    }
...
let GetArmstrongNumber = (_num) => {
    return new Promise(function (resolve, reject) {
        let _armstrong_number = 0;
        let _num_temp = 0;
        let _num_output = _num;
